...
class MinimalAlarmService {
  static const String channelId = 'guardian_paws_minimal';
  static const String channelName = 'Guardian Paws Minimal Protection';
  ...
  static Future<void> initialize() async {
    ...
    try {
      ...
      print('Error initializing MinimalAlarmService: $e');
      _isInitialized = true;
    }
  }
  ...
  static Future<void> startPersistentProtection({
    ...
    try {
      ...
      await prefs.setBool('minimal_protection_active', true);
      ...
      print('Minimal protection mode started with ${intervalMinutes} minute intervals');
    } catch (e) {
      print('Error starting minimal protection mode: $e');
      throw Exception('Failed to start minimal protection: $e');
    }
  }
  
  static void _startSimpleTimer(int intervalMinutes) {
    print('Starting minimal timer for ${intervalMinutes} minutes');
    ...
    _protectionTimer = Timer.periodic(Duration(minutes: intervalMinutes), (timer) async {
      try {
        if (_isProtectionActive) {
          print('Minimal timer triggered - showing safety check');
        ...
        print('Error in minimal timer: $e');
      }
    });
  }
  ...
  static Future<void> stopPersistentProtection() async {
    try {
      ...
      final prefs = await SharedPreferences.getInstance();
      await prefs.setBool('minimal_protection_active', false);
      
      _isProtectionActive = false;
      print('Minimal protection mode stopped');
    } catch (e) {
      print('Error stopping minimal protection mode: $e');
    }
  }
  ...
  static Future<bool> isPersistentProtectionActive() async {
    try {
      final prefs = await SharedPreferences.getInstance();
      return prefs.getBool('minimal_protection_active') ?? false;
    } catch (e) {
      print('Error checking minimal protection status: $e');
      return false;
    }
  }
  ...
  static Future<void> _showAlarmOverlay() async {
    try {
      print('Minimal alarm triggered - showing safety check');
      ...
      print('Error showing minimal alarm overlay: $e');
    }
  }
